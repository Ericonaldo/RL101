...
class DeepQNetwork():
    def __init__(self, env, sess=None, gamma = 0.8, epsilon = 0.9):
        self.gamma = gamma
        self.epsilon = epsilon
        self.actlow = env.action_space.low
        self.acthigh = env.action_space.high
    ...
    def network(self):
        ...
        with tf.variable_scope("train"):
            self.train_op = tf.train.RMSPropOptimizer(0.005).minimize(self.loss)
    ...
    with tf.Session() as sess:
        ...
        for episode in range(EPISODES):
            ...
            while True:
                ...
                #action_t = action*(env.action_space.high[0]-env.action_space.low[0])/(ACTION_DIM)+env.action_space.low[0]
                action_t = (action-(ACTION_DIM-1)/2)/((ACTION_DIM-1)/4)   # convert to [-2 ~ 2] float actions
                next_state, reward, done , _ = env.step([action_t])
                reward_all += reward

                #if len(memory) > MEMORY_SIZE:
                #    memory.pop(0)
                if len(memory) - MEMORY_SIZE > 20000:
                    break
